...
const BredCrumb: React.FC<any> = ({ title, dropdown, setDropdown }) => {
    const [sort, setSort] = useState(false)

    return (
        <div className="mb-0 py-4 w-full flex justify-between items-center">
            ...
            <div onClick={() => setDropdown(!dropdown)} className="flex items-center justify-center px-4 py-2"> Filters  <BsChevronDown className={`transform ${dropdown ? 'rotate-180' : ''} ml-2`} size={15} />

            </div>
            <div
                tabIndex={-1}
                className={`fixed top-0 left-0 right-0 bottom-0 z-10 duration-300 transition-all ease-in-out delay-75 bg-white h-full  ${dropdown === true ? 'translate-y-0 overflow-y-scroll' : 'translate-y-full'
                    }`} >
            ...
            {/* <div className={`${dropdown === true ? " bg-black absolute top-0 left-0 bottom-0 right-0 z-10" : ""} `} onClick={() => setDropdown(!dropdown)}></div> */}
